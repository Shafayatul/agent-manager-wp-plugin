<?php
ob_start();
function camOrderTourism_tripListFunction(){
	$cam_display_order_tourism_trip_table = new Cam_Order_tourism_trip_List_Table();
	$cam_display_order_tourism_trip_table->prepare_items();
	?>
		<div class="wrap">
			<div id="icon-users" class="icon32"></div>
			<h2>Tourism Trip Orders</h2>
			<form method="post">
				<input type="hidden" name="page" value="example_list_table" />
			</form>
			<?php $cam_display_order_tourism_trip_table->display(); ?>
		</div>
	<?php
}
...
class Cam_Order_tourism_trip_List_Table extends WP_List_Table {
	...
	private function table_data() {
		...
		$orders          = $wpdb->get_results( "SELECT * FROM ".$table_name." WHERE order_type='tourism_trip'", OBJECT );
